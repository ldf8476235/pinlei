-- diagnosis GMROI 预计算结果层升级脚本 (SQL Server)

SET ANSI_NULLS ON;
SET QUOTED_IDENTIFIER ON;
GO

IF OBJECT_ID(N'dbo.diag_result_gmroi_sku', N'U') IS NULL
BEGIN
    CREATE TABLE dbo.diag_result_gmroi_sku
    (
        ...
        compare_turnover_rate  DECIMAL(20,4)  NOT NULL DEFAULT 0,
        compare_gross_rate     DECIMAL(20,2)  NOT NULL DEFAULT 0,
        compare_gmroi          DECIMAL(20,4)  NOT NULL DEFAULT 0,
...
IF NOT EXISTS (
    ...
    WHERE name = N'ux_diag_result_gmroi_sku_query_ver_product'
      AND object_id = OBJECT_ID(N'dbo.diag_result_gmroi_sku')
)
BEGIN
    CREATE UNIQUE INDEX ux_diag_result_gmroi_sku_query_ver_product
        ON dbo.diag_result_gmroi_sku(tenant_id, query_hash, data_version, product_no);
END
GO

IF NOT EXISTS (
    SELECT 1 FROM sys.indexes
    WHERE name = N'ix_diag_result_gmroi_sku_filter'
      AND object_id = OBJECT_ID(N'dbo.diag_result_gmroi_sku')
)
BEGIN
    CREATE INDEX ix_diag_result_gmroi_sku_filter
        ON dbo.diag_result_gmroi_sku(
            tenant_id, query_hash, data_version,
            current_gmroi_role, compare_gmroi_role, product_status_no, promotion_flag
        );
END
GO

PRINT N'[OK] diagnosis gmroi schema upgrade done';
GO
